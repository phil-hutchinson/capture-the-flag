...
- **Location** — a single markdown link combining file path and line number(s), e.g. `[capture_the_flag/board.py#L12](../../../capture_the_flag/board.py#L12)` or `#L42-L50` for a range. The link must be a **relative path from the peer review file's location** (`doc/plan/<story>/peer-review.md`), so paths to project source files will typically start with `../../../`.
