...
def run_batch(
    ...
    for game_number in range(1, num_games + 1):
        white = RandomCtfPlayer("Random White", rng)
        black = RandomCtfPlayer("Random Black", rng)
        match_result = play_match(white, black, render_final_board=False)

        outcome = match_result.game_result.outcome
        if outcome == 1:
            white_wins += 1
        elif outcome == -1:
            black_wins += 1
            ...
            draws += 1
        ply_counts.append(len(match_result.game_result.game_log))
