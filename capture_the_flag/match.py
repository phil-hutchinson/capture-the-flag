...
def build_initial_position(
    ...
    `layout` says which board is being played. The library's `position_factory`
    contract fixes the two-player signature, so a caller binds the layout ahead of
    time (`functools.partial`) rather than passing it per game — every game in a
    run is played on one board.
    """
    ...
    with region(STARTING_POSITION):
        white_placement = cast(CtfPlayer, side_one).get_placement(Side.WHITE, layout)
        black_placement = cast(CtfPlayer, side_other).get_placement(Side.BLACK, layout)
        return assemble_position(white_placement, black_placement, layout)
...
def play_match(
    ...
    """
    white_placement = white_player.get_placement(Side.WHITE, layout)
    black_placement = black_player.get_placement(Side.BLACK, layout)
    initial_position = assemble_position(white_placement, black_placement, layout)
