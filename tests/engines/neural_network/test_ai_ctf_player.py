...
`StandardGame` wrapper — with an untrained `AICtfPlayer` against a random
...
def test_ai_player_completes_full_match_against_random():
    ...
    ai_player = AICtfPlayer(engine, name="ai")
    random_player = RandomCtfPlayer(name="random", rng=random.Random(1234))

    result = play_match(ai_player, random_player).game_result
